...
def test_save_token(repo, mock_session, existing_token):
    ...
    if existing_token:
        ...
        # Check that add was called with a BankItemToken instance
        assert mock_session.add.call_count == 1
        added_obj = mock_session.add.call_args[0][0]
        assert isinstance(added_obj, BankItemToken)
...
def test_save_cursor(repo, mock_session, existing_cursor):
    ...
    if existing_cursor:
        ...
        # Check that add was called with a BankItemCursor instance
        assert mock_session.add.call_count == 1
        added_obj = mock_session.add.call_args[0][0]
        assert isinstance(added_obj, BankItemCursor)
